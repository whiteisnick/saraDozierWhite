	...
	$('#back').on('click', function() {
		if (currentSample > 1) {
			if ((currentSample === 2) || (currentSample === 5)) {
				...
				$('.samples-photo').removeClass('col-xs-8 col-xs-offset-2').addClass('col-xs-10 col-xs-offset-1');
				...
				currentSample = currentSample - 1;
				$('.samples-photo').toggle();
				$('.samples-photo').removeClass('col-xs-10 col-xs-offset-1').addClass('col-xs-8 col-xs-offset-2');
				$('#sample').html('<img class="img-responsive-height img-rounded" src="images/sample' + currentSample + '.jpg" alt="Responsive image">');
				$('.samples-photo').fadeToggle(700);
				return(currentSample);
			} else {
